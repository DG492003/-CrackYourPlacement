...
class Solution {
private:
    // Function to calculate the minimum number of elements to form the target
    // sum
    int minimumElementsUtil(vector<int>& arr, int ind, int T, vector<vector<int>>& dp) {
        ...
        if (ind == 0) {
            ...
            // If yes, return the quotient as the answer
            if (T % arr[0] == 0)
                return T / arr[0]; 
            // Otherwise, return a very large value to indicate
            // it's not possible
            else
                return 1e9; 
        }
        ...
        // Calculate the minimum elements needed without taking the current
        // element
        ...
        // Store the minimum of 'notTaken' and 'taken' in the DP array and
        // return it
        return dp[ind][T] = min(notTaken, taken);
    }

public:
    // Function to find the minimum number of elements needed to form the target
    // sum
    int coinChange(vector<int>& arr, int T) {

        int n = arr.size();

        // Create a DP (Dynamic Programming) table with n rows and T+1 columns
